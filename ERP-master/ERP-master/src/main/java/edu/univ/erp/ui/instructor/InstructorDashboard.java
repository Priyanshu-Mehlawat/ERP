...
public class InstructorDashboard extends JFrame {
    ...
    private boolean loadCurrentInstructor() {
        try {
            ...
            if (userRole != null && userRole.equals(UserRole.INSTRUCTOR)) {
                ...
                // Store instructor ID in session for permission checking
                SessionManager.getInstance().setInstructorId(currentInstructor.getInstructorId());
                logger.info("Instructor ID {} stored in session for user {}", currentInstructor.getInstructorId(), currentUser.getUsername());
                return true;
            } else {
    ...
    private void openDialog(String title, int width, int height, JPanel panel) {
        ...
        dialog.setMinimumSize(new Dimension(600, 400)); // Set minimum size
