    ...
    private void loadStudentId() {
        try {
            ...
            if (userId != null) {
                ...
                if (student != null) {
                    SessionManager.getInstance().setStudentId(student.getStudentId());
                    logger.info("Loaded student ID {} for user ID {}", student.getStudentId(), userId);
                } else {
                    logger.warn("No student record found for user ID: {}", userId);
                }
            }
        } catch (Exception e) {
            logger.error("Error loading student ID", e);
        }
    }
